...
export function WaitTimeChart({ data }: WaitTimeChartProps) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="p-3 rounded-lg shadow-lg" style={{ backgroundColor: '#1a1a1a', border: '1px solid #2a2a2a' }}>
          <p className="font-medium text-white">{item.gemeente}</p>
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-[20px] p-6 border border-[#2a2a2a] h-full">
      <h3 className="text-lg font-semibold text-white mb-4">Gemiddelde Wachttijd per Gemeente</h3>
      ...
      <div className="mt-4 flex justify-center gap-6 text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[#888888]">≤2 dagen</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full bg-amber-500" />
          <span className="text-[#888888]">3-4 dagen</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full bg-red-500" />
          <span className="text-[#888888]">5+ dagen</span>
        </div>
      </div>
    </div>
  );
