    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-subtle)', color: 'var(--text-primary)' }} className="p-3 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-text-secondary">Inkomen: {formatCurrency(item.inkomen)}</p>
          <p className="text-sm text-text-secondary">Schuld: {formatCurrency(item.schuld)}</p>
  ...
  return (
    <div className="bg-card rounded-[20px] p-6 shadow-sm border border-border-subtle h-full">
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-lg font-semibold text-foreground">Schuld vs. Inkomen</h3>
